...
abbreviation = ["u.s.a", "fig", "Fig","et al", "i.e", "e.g", "ref", "c.f", "Eq", "e.q", "eqn", "Eqn", "dr", "Dr","spp","u.s","sp", "etc"]
                    ...
                    for m in range(len(intersect)):
                        ...
                        for s in range(len(tmp_t1)):
                            for r in range(len(tmp_t2)):
                                if abs(int(tmp_t1_d[s]) - int(tmp_t2_d[r])) < min:
                                    min  = abs(int(tmp_t1_d[s]) - int(tmp_t2_d[r]))
                                    mind_ind = r
                            #print(min)


                            the_file.write('R{}\tExhibits Microorganism:{} Property:{}\n'.format(num_lines + m + q + s + 1, tmp_t1[s],tmp_t2[mind_ind]))
                        q = q + s
                ...
                with open(db.file_names_ending_with_a1[i][:-3] + ".a2", 'a+') as the_file:
                    ...
                    for m in range(len(intersect)):
                        ...
                        for r in range(len(tmp_t2)):
                            ...
                            the_file.write('R{}\tLives_In Microorganism:{} Location:{}\n'.format(num_lines + m + q + r + 1, tmp_t1[mind_ind],tmp_t2[r]))
                        q = q + r
...
def Find_Relationship_Deltas(deltas_helper_list, db):
    ...
    for i in range(dim_x):
        ...
        for z in range(length-1):
            ...
            if list(db.deltas_helper_list[i].values())[z] == ["Microorganism"] and "Habitat" in list(db.deltas_helper_list[i].values())[z+1] and len(set(list(db.deltas_helper_list[i].values())[z+1])) == 1 and len(list(db.deltas_helper_list[i].values())[z+1])>1 and (int(list(db.deltas_helper_list[i].keys())[z+1][9:])-int(list(db.deltas_helper_list[i].keys())[z][9:])) < 3:
                ...
                with open(db.file_names_ending_with_a1[i][:-3] + ".a2", 'a+') as the_file:
                        ...
                        for j in range(dim_y):
                                ...
                                if (tmp_1 == db.file_contents_a1_with_sentenceID_shotened[i][j].split()[1])  and (list(db.deltas_helper_list[i].keys())[z] == db.file_contents_a1_with_sentenceID_shotened[i][j].split()[-1]):
                                    tmp_t1.append(db.file_contents_a1_with_sentenceID_shotened[i][j].split()[0])
  
                                    
                                if (tmp_2 == db.file_contents_a1_with_sentenceID_shotened[i][j].split()[1]) and (list(db.deltas_helper_list[i].keys())[z+1] == db.file_contents_a1_with_sentenceID_shotened[i][j].split()[-1]) :
                                    tmp_t2.append(db.file_contents_a1_with_sentenceID_shotened[i][j].split()[0])
                        ...
                        for s in range(len(tmp_t2)):
                            ...
                            the_file.write('R{}\tLives_In Microorganism:{} Location:{}\n'.format(num_lines + q + s + 1, tmp_t1[0],tmp_t2[s]))
                        q = q + s


                
            if list(db.deltas_helper_list[i].values())[z] == ["Microorganism"] and "Geographical" in list(db.deltas_helper_list[i].values())[z+1] and len(set(list(db.deltas_helper_list[i].values())[z+1])) == 1 and len(list(db.deltas_helper_list[i].values())[z+1])>1 and (int(list(db.deltas_helper_list[i].keys())[z+1][9:])-int(list(db.deltas_helper_list[i].keys())[z][9:])) < 3:
                print(db.file_names_ending_with_a1[i])
                print("HEYYYYYYYY")
